...
**KOI Fase 2 — street sales (built 2026-07-23, pending Jess's local validation)**: tables `koi_sales` + `koi_sale_items` (migration `0003`), where prices and costs are **snapshotted** per line so later catalog edits never rewrite history. A shift is registered two ways, both writing the same rows: the Discord panel (`/koi-setup` publishes it; 💰 opens a modal with one quantity field per dish — a select picks up to 5 first when the catalog outgrows a modal) and the dashboard (KOI → aba **Vendas de rua**: date + quantities). Both price the shift from the configured street prices via `priceSaleItems`, and estimated profit uses the collecting cost. Bot also has 📊 Semana / 🧾 Minhas vendas buttons, an optional record channel (`/koi-log-setup`) and an **automatic Monday 09:00 post** of the closed week (`weekly-post.ts`, restart-safe: the posted week is marked in `settings`). Dashboard shows revenue/profit/rankings/history per period (7/30/90 days) plus KOI sales cards and a per-day chart on the home.
...
- `src/koi/` — KOI restaurant module: `types.ts`, `pricing.ts` (pure margin math **and** `priceSaleItems`, shared with the web), `sales-summary.ts` (pure aggregation, shared), `catalog.ts` + `sales-repository.ts` (DB access, bot/test side only), `panel.ts`/`ids.ts`/`sale-modal.ts`/`handlers.ts`/`summary-view.ts`/`sales-log.ts` (Discord), `week.ts` + `weekly-post.ts` (Monday summary schedule).
...
- **Show/edit page pattern** (settled 2026-07-23, use for every new one): `<PageBreadcrumbs>` (trail with `›` separator, links on the navigable levels) then `<EntityCard>` — one full-width card wrapping the whole content, with an h5 title (matching index pages), optional subheader and header action (e.g. a status chip), a divider, then the content (fields in a MUI `Grid`, Salvar bottom-right). See `matriculas/[passport]` (show) and `koi/pratos|ingredientes/[id]/editar` (edit).
- **Tab-in-URL**: when a page has tabs and links point back to a specific tab, keep the active tab in the query string (e.g. `/koi?tab=ingredientes`) so breadcrumbs and post-save redirects land on the right tab — not React state (which resets on navigation). See `koi-view.tsx`.
- KOI index uses **per-dish cards** (`koi-margins-cards`) not a dense table: each card labels cost/profit per batch for buying-vs-collecting (collecting emphasized), a street-price simulator, and an edit link. Dish/ingredient emojis (approximating the in-game icons) live in `web/src/koi-icons.ts`.
...
- **Server/client boundary — two rules that already bit us**: (1) server components must not pass functions (e.g. `component={Link}`) to client components — wrap the pairing in a client component like `button-link.tsx` or `due-queue-list.tsx`; (2) a server component must not import a **runtime value** from a `'use client'` module (it receives a client reference, not the value — `SALES_PERIODS.find is not a function`). Shared constants live in a neutral module such as `sales-periods.ts`; type-only imports are always fine. Neither mistake fails the build — both blow up at request time, so exercise the page locally.
